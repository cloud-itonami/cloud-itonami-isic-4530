(ns autoparts.operation
  "OperationActor -- one auto-parts-retail coordination operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (PartsOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the AutoPartsOpsGovernor (:govern)
  ...
  One graph run = one coordination operation (intake -> advise -> govern
  -> decide -> commit | hold | approval). No unbounded inner loop --
  each operation is auditable and checkpointed. A storefront's
  operating history is advanced by MANY ops (sales logging / restock
  scheduling / concern flagging / supply-order coordination), each its
  own independent run.
  ...
  :approved}}` (or `:rejected`). `:flag-compatibility-concern`
  ALWAYS reaches this node -- see `autoparts.phase`/`autoparts.governor`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [autoparts.governor :as governor]
            [autoparts.partsopsadvisor :as partsopsadvisor]
...
(defn build
  ...
  opts:
    :advisor      -- a `autoparts.partsopsadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (partsopsadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PartsOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (partsopsadvisor/-advise advisor store request)]
            {:proposal p :audit [(partsopsadvisor/trace request p)]})))

      ;; AutoPartsOpsGovernor -- independent censor (separate system than the LLM).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:always-escalate? verdict) :compatibility-concern
                                          (:high-cost? verdict) :cost-threshold
                                          :else :low-confidence))
                        :phase ph
